...
class BaseLine:
    ...
    def __init__(self, image, start_xy, iterations):
        ...
        self.line_color = (220, 220, 200)


        for i in range(len(self.COORDS)+1):
            if i <= len(self.COORDS):
                connection_coords = self.generate_connecting_coords(self.COORDS[i], self.COORDS[i + 1])
    ...
    def generate_coords(self, max_iter: int):
        ...
        for i in range(max_iter):
            coord = (
                self.start_xy[0] + (random.randint(10, 100) * 10),
                self.start_xy[1] + (random.randint(10, 100) * 10)
            )
    ...
    @staticmethod
    def generate_connecting_coords(xy1, xy2):
